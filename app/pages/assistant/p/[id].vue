<template>
    <div class="relative flex flex-col h-full bg-gray-50">
        ...
        >
          <div
            :class="[
              ...
              msg.sender === 'user' ? 'bg-[#10b481] text-white rounded-br-none' : 'bg-white text-gray-800 rounded-bl-none'
            ]"
          >
      ...
      <!-- Modules selection -->
      <div class="absolute left-0 right-0 bottom-24 p-4 bg-white border-t shadow-lg flex flex-wrap gap-3">
        <label 
        ...
        >
          <input type="checkbox" v-model="selectedModules" :value="module" class="accent-green-500"/>
  ...
  const modulesList = ['Parcel', 'Soil', 'Taches', 'Culture', 'Meteo', 'Historique de rendement']
  ...
  // Transform selectedModules (array) into an object for backend
